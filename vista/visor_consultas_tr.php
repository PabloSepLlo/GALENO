...
<body>
    <?php 
        ...
        if (isset($_GET["filtroTR"])) {
            echo "
                <div class='modal fade show' id='modalFiltroTR' tabindex='-1' style='display: block; background: rgba(0,0,0,0.5);' aria-labelledby='modalFiltroTRLabel'>
                    <div class='modal-dialog modal-dialog-centered'>
                        <div class='modal-content'>
                            <div class='modal-header'>
                                <h5 class='modal-title'>Seleccione el tratamiento de los pacientes que quiere consultar</h5>
                                <a href='./menu.php' class='btn-close'></a>
                            </div>
                            <div class='modal-body'>
                                <form method='POST' action='../controlador/cargar_datos_paciente_por_tr.php'>
                                        ...
                                        <select class='form-select' id='tratamiento' name='tratamiento'>
                                            <option value=''>-</option>";
                                                ...
                                                foreach ($_SESSION["datos_tr"] as $tr) {
                                                    echo "<option value='{$tr["id_tratamiento"]}'>{$tr["descripcion"]}</option>";
                                                }
                                                unset($_SESSION["datos_tr"]);
                                        echo "</select>
            ...
            unset($_SESSION["pacientes_por_migr"], $_SESSION["pacientes_por_cs"], $_SESSION["pacientes_por_tr"]);
        }
        if (isset($_SESSION["pacientes_por_tr"])) {
            echo "
                        ...
                        <div class='card-body py-3'>
                            <div class='d-flex flex-column flex-md-row justify-content-center align-items-center'>
                                <h3 class='card-title mb-2 fw-bold titulo-azul-oscuro'>
                                    PACIENTES CON TRATAMIENTO: {$_SESSION["pacientes_por_tr"][0]["tratamiento"]}
                                </h3>
                            </div>
                        </div>
                    </div>
            ...
            <div class='row d-flex justify-content-between align-items-center my-2 mx-5'>
            ...
            foreach ($_SESSION["pacientes_por_tr"] as $paciente) { 
                echo "<tr>
                        <td>{$paciente['nhc']}</td>
